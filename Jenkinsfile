pipeline {
    ...
    parameters {
       ...
       gitParameter branch: '', branchFilter: 'origin/(.*)', defaultValue: 'master', description: 'Select branch to build', name: 'BRANCH', 
	              quickFilterEnabled: false, selectedValue: 'DEFAULT', 
        ...
        stage('Build') {
		    ...
		    //if (parentPom.version == RELEASE_VERSION) {
		        // add version number to build displayName
                        //currentBuild.displayName = "#${currentBuild.number}_v${parentPom.version}"
